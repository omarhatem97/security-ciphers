...
def inv_sbox(out_shift):
    ...
    for i in range(4):
        for j in range(4):
            curr = int(out_shift[i][j], 16)

            res[i][j] = f'{isbox[curr]:02x}'
    return res
...
def decrypt_aes(key, cipher):
    """takes 32-long cipher string , key, returns plain text"""
    plain = ''
    ...
    #Round 0
    #convert palain text to state matrix
    state = convert_to_state(cipher)

    state = add_round_key(state, convert_to_state(keys[0]))

    for i in range(10):
        if(i==0):
            state = state
        else:
            state = add_round_key(state, convert_to_state(keys[i]))
...
if __name__ == '__main__':
    ...
    # e = shift_rows(d)
    # # print(e)
    # # print(inv_shift_rows(e))
    # # e = list_to_int(e)
    # e = transpose_matrix(e)
    # print(e)
    # d = inv_mix_coloumns(mix_coloumns(e))
    # print(d)
